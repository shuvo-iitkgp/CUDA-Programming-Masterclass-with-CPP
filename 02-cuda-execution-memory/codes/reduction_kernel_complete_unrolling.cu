#include <stdio.h>
#include <cuda_runtime.h>
#include "device_launch_parameters.h"
#include "common.h"

__global__ void gpuRecursiveReudce(int *g_idata, int *g_odata, unsigned int size) {
  int tid = threadIdx.x; 
  int *idata = g_data + blockIdx.x * blockDim.x ; 
  int *odata = &g_odata[blockIdx.x];

  if (isize == 2 && tid ==0 ){
    g_odata[blockIdx.x] = g_idata[0] + g_idata[1]; 
    return; 
  
  }
  int istride = isize >> 1; 
  if (istride > 1 && tid < istride){
    idata[tid] += idata[tid + stride]; 
  }
  __syncthreads(); 
  if (tid == 0){
    gpuRecursiveReudce<<1, istride>>>(idata, odata, istride); 
    cudaDeviceSynchronize(); 
  }
  __syncthreads(); 
}



int main() {
    printf("=== GPU Parallel Reduction ===\n");

    int size = 1024;
    int block_size = 128;
    int byte_size = size * sizeof(int);
    int grid_size = (size + block_size - 1) / block_size;

    // Host allocations
    int *h_input = (int*)malloc(byte_size);
    int *h_temp  = (int*)malloc(grid_size * sizeof(int));

    // Initialize
    initialize(h_input, size, INIT_ONE);
    int cpu_result = reduction_cpu(h_input, size);

    // Device allocations
    int *d_input, *d_temp;
    cudaMalloc((void**)&d_input, byte_size);
    cudaMalloc((void**)&d_temp, grid_size * sizeof(int));
    cudaMemcpy(d_input, h_input, byte_size, cudaMemcpyHostToDevice);

    // Launch kernel with dynamic shared memory
    gpuRecursiveReudce<<<grid_size, block_size, block_size * sizeof(int)>>>(d_input, d_temp, size);
    cudaDeviceSynchronize();

    // Copy partial sums to host
    cudaMemcpy(h_temp, d_temp, grid_size * sizeof(int), cudaMemcpyDeviceToHost);

    // Final reduction on CPU
    int gpu_result = 0;
    for (int i = 0; i < grid_size; ++i) {
        gpu_result += h_temp[i];
    }

    compare_results(gpu_result, cpu_result);

    // Cleanup
    free(h_input);
    free(h_temp);
    cudaFree(d_input);
    cudaFree(d_temp);

    return 0;
}
